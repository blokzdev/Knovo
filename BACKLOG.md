...
| **Audience signal — privacy-first analytics (Phase 2)** | ✅ Delivered (2026-06-26, `0011`): server-side, **cookieless, no-PII** view/return measurement on published artifacts, surfaced as an **Audience** section in `/admin/insights` (views/day, unique + returning readers, top artifacts). Readers are keyed by `HMAC(salt, ip+ua)` computed inside a `SECURITY DEFINER` recorder; the salt rotates + is destroyed every 7-day window (so "returning" is a within-the-week signal) and **IP/UA are never stored**. Recording runs in the `/a/[slug]` server render (`lib/audience/record.ts`), skipping bots + prefetches; tables carry no browser RLS grant (service-role read, server-side aggregation in unit-tested `lib/admin/audience.ts`). Outside the worker API + slot schema → no `docs/routines.md` change. Answers the open "are practitioners finding + returning?" question (the *threshold* for "validated" stays open). | — (done) |
