...
| 5 | **Public reader accounts are in scope.** *(Amended 2026-06-22.)* Optional Google sign-in for readers, with **bookmarks, public reader comments (with admin moderation), and subscribe** (RSS + recorded intent now; transactional email later). Reading stays fully open/account-free. Reader tables (`bookmarks`, `reader_comments`, `subscriptions`, `public_profiles` view) live in `0005`, OUTSIDE the worker API + slot schema. *(The admin-only editorial **comments/directives** in 0004 are internal control signals — distinct from public `reader_comments`.)* | Dogfood as reader + admin and seed engagement now that authoring is live; readers ≠ broad-biomedical scope creep. | Locked (amended) |
